...
void EmulatedUSBDeviceFrame::OnSkylanderCommand(wxCommandEvent& event)
{
	int slot = event.GetInt();
	wxString path = event.GetString();

	// Controlla il tipo di comando basato sul contenuto della stringa
	if (path.IsSameAs("CLEAR", false))
	{
		// Se la stringa è "CLEAR", esegui il comando di cancellazione
		ClearSkylander(slot);
	}
	else
	{
		// Altrimenti, il comando è "SUMMON" e il percorso è valido
		LoadSkylanderPath(slot, path);
	}
}
			  ...
			  wxDefaultSize, wxDEFAULT_FRAME_STYLE | wxTAB_TRAVERSAL)
{
	...
	// Avvia il server e gestisce gli eventi
	m_skylanderServer = new SkylanderServer(this);
	m_skylanderServer->Start();

	Bind(wxEVT_SKYL_COMMAND, &EmulatedUSBDeviceFrame::OnSkylanderCommand, this);

	Bind(wxEVT_CLOSE_WINDOW, [&](wxCloseEvent& event) {
		if (m_skylanderServer)
		{
			m_skylanderServer->Delete();
			m_skylanderServer = nullptr;
		}
		event.Skip();
	});
}
...
{
	if (slot >= 0 && slot < m_skylanderSlots.size())
	{
		if (auto slotInfos = m_skySlots[slot])
		{
			auto [curSlot, id, var] = slotInfos.value();
			// Rimuovi la figura dal portale emulato
			nsyshid::g_skyportal.RemoveSkylander(curSlot);
			// Azzera i dati dello slot in memoria
			m_skySlots[slot] = {};
		}
		// Forza l'aggiornamento del campo di testo dell'interfaccia utente.
		// Imposta il testo a vuoto per riflettere il comando "CLEAR".
		m_skylanderSlots[slot]->SetValue("");
		// Chiamata all'aggiornamento generale per riflettere eventuali modifiche
		// che non sono gestite esplicitamente sopra.
		UpdateSkylanderEdits();
	}
}
